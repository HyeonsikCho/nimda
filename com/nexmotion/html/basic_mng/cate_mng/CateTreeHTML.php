...
function getCateTopTree($one_level, $two_level, $thr_level, $func, $flag=TRUE) {
    ...
    foreach ($one_level as $key=>$value) {
        ...
        $html .= "\n         <li class=\"ptb05\">"; 
        $html .= "\n             <label onclick=\"" . $func[1] . "('#" . $key . "', '" . $value . "', '" . $key . "' ,'select');\" for=\"folder" . $i . "\"><span class=\"one\" id=\"" . $key . "\">" . $value. "</span>";
...
function getMidTree($key, $two_level, $thr_level, $func, $i, $flag) {
    ...
    if ($two_level[$key]) {
        foreach ($two_level[$key] as $arr_key=>$value) { 
            ...
            $html.= "\n                 <li>";
            $html.= "\n                     <label for=\"subfolder" . $j . "\"onclick=\"" . $func[2] . "('#" . $arr_key . "', '" . $value . "', '" . $arr_key . "', 'select');\"><span class=\"two\" id=\"" . $arr_key . "\">" . $value . "</span>"; 
            if ($flag === TRUE) {
...
function getBtmTree($key, $thr_level, $func, $flag) {
    ...
    if ($thr_level[$key]) { 
        foreach ($thr_level[$key] as $arr_key=>$value) {
            ...
            $html .= "\n                        <li class=\"file\"><a style=\"cursor:pointer;\" class=\"thr\" id=\"" . $arr_key ."\" onclick=\"" . $func[3] . "(this, '" . $value . "', '" . $arr_key . "', 'select');\">" . $value . "</a></li>";
        }
    }
