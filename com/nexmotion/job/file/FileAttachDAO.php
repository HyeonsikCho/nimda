...
class FileAttachDAO {
	...
	function chkPath($path){
		...
		for ( $x = 0; $x < count($tmp); $x++ ) {
            ...
            if (!mkdir(SELF::DEFAULT_PATH . $chkdir, 0777, true))  {
                return false;
            }
		}
    ...
    function checkExcelFiles($files, $name) {
        if (!$this->checkUploadFiles($files, $name)) {
            return false;
        }
     ...
     * @return 파일이 제대로 존재하면 true 아니면 false
     */
    function checkUploadFiles($files, $name) {
