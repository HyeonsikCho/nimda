...
var cndSearch = {
    "exec"       : function(change) {
        var url = "/ajax/produce/process_result/load_dlvr_waitout_op_process_result_list.php";
        ...
        var callback = function(result) {
            ...
            for(i = 1 ; i <= length; i++) {
                ...
                if (a_obj.order_num != null) {
                    ...
                    if(parseInt(a_obj.order_state) < 3220) {
                        force = true;
                        ...
                        $("#stock_cancel_" + i).attr({"class" : "btn_dw_nothing btn" ,
                            "disabled" : true});
                    } else if(parseInt(a_obj.order_state) == 3220) {
                ...
                if(data.order_detail_num == a_obj.order_detail_num) {
                    ...
                    if(force == true) {
                        msg_alert = "해당 상품 구성 중 출고 완료처리가 되지 않은 상품이 있습니다.\n"
...
var processStock = {
    "exec"       : function() {
        var url = "/ajax/produce/process_result/load_dlvr_stockout_process.php";
        var blank = "<tr><td colspan=\"20\">검색 된 내용이 없습니다.</td></tr>";
        var data = {
...
var findButton = function(change) {
    ...
    if(order_no == $("#recorded_barcode").val() && Number($("#recorded_barcode").attr("state")) <= Number("3220")) {
        change = "Y";
    } else {
        change = "N";
    }
...
function btn_stock_force(order_num) {
    ...
    findButton("Y");
    /*
     var url = "/ajax/produce/process_mng/delivery_waitout_stock_force.php";
     var data = {
...
function btn_stock_cancel(order_num) {
    ...
    if(!confirm("해당 상품 구성의 출고를 취소 하시겠습니까?"))
    {
        return;
    }

    var url = "/ajax/produce/process_mng/delivery_waitout_stock_cancel.php";
    var data = {
        "order_detail_num"       : order_num
    };
    var callback = function(result) {
...
function alertShowMask() {
    ...
    html += "해당 상품 구성 중 입고처리가 되지 않은 상품이 있습니다.\n 주문내역과 상이한지 확인 후 진행하시기 바랍니다.</div>";
    openRegiPopup(html, 470);
}
