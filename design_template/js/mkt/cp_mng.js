...
var saveCpInfo = function(seq) {
    ...
    //발행일 
    var start_date = $("#public_period_start_date").val();
    var end_date = $("#public_period_end_date").val();
    ...
    //사용시간
    if ($("input:checkbox[name='usehour_yn']").is(':checked') == true) {
        //사용여부가 체크된 경우 Y 로 설정
        $("input:checkbox[name='usehour_yn']").val('Y');
    ...
    //만료일 (발행후, 날짜지정, 만료일없음)
    if ($("input:radio[name='expire_dvs']:checked").val() == 1) {
        //발행 후 유효일이 비었을때
        if ($("#expire_public_day").val() == "") {
            alert("발행 후 유효일을 입력해주세요.");
            $("#expire_public_day").focus();
            return false;
        }

    } else if ($("input:radio[name='expire_dvs']:checked").val() == 2) {
        //소멸날짜가 비었을때
        if ($("#expire_extinct_date").val() == "") {
            alert("소멸날짜를 입력해주세요.");
            $("#expire_extinct_date").focus();
            return false;
        } else {
            if ($("input:radio[name='public_dvs']:checked").val() == 1) {
                var expire_extinct_date = $("#expire_extinct_date").val();
                if (start_date > expire_extinct_date) {
                    alert("소멸일자가 시작일보다 빠릅니다" + 
                            "\n날짜를 확인해주세요.");
                    return false;
                }
            }
        }
    } else if ($("input:radio[name='expire_dvs']:checked").val() == 3) {
        //만료일은 특별한 작업 없음
    }

    //대상미지정일 경우 발행수량 확인
    if ($("input:radio[name='object_appoint']:checked").val() == 'N') {
        if ($("#public_amt").val() == "" || 
            $("#public_amt").val() ==null || 
            $("#public_amt").val() <= 0) {
            alert("발행매수를 입력해주세요.");
    ...
    if ($("#upload_file").val() == '' || $("#upload_file").val() == null) {
        if ($("#file_seqno").val() == '' || $("#file_seqno").val() == null) {
