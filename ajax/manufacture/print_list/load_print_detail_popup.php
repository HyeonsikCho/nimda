...
$paper_stor_yn = $rs->fields["paper_stor_yn"]; 
$html_param["paper_stor_yn"] = $paper_stor_yn; 
$html_param["size_info"] = $rs->fields["affil"] . " " . $rs->fields["subpaper"] . "(" . $rs->fields["size"] . ")";
...
$memo_select_html = <<<HTML
                             ...
                             <input type="text" class="input_dis" style="width:78%;" id="worker_memo" disabled="disabled" value="$param[worker_memo]">
HTML;

if ($rs->fields["state"] == $state_arr["인쇄대기"]) {
    if ($paper_stor_yn != "Y") {
        $btn_html = <<<HTML
              <label class="btn btn_md fix_width180" onclick="paperStor('$seqno');" id="paper_use_btn">종이사용</label>
HTML;
    }
    $btn_html .= <<<HTML
...
echo getPrintDatilPopup($html_param); 
$conn->close();
?>
