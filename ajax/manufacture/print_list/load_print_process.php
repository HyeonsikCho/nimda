...
//임시
$tmp = explode("-", $fb->form("typset_num"));
...
if ($flattyp_dvs == "Y") {
    ...
    $param["col"] = "opt_list ,sheet_typset_seqno, oper_sys
        ...
        ,aftside_spc_tmpt, dlvrboard, specialty_items";
    ...
    while ($picture_rs && !$picture_rs->EOF) {
        ...
        if (is_file($chk_path) === false) {
            $full_path = NO_IMAGE;
            $html_param["pic"] .= "<td colspan=\"2\" style=\"text-align: center;\"><img src=\"" . $full_path . "\" style=\"width:840px; height:600px;\"></td>"; 
        } else {
