...
*/
$param["dvs"] = "SEQ";
$rs = $dao->selectMtraStockMngDetailList($conn, $param);
$list = "";
while ($rs && !$rs->EOF) {
    ...
    if ($rs->fields["realstock_amt"])
        $btn = "<button type=\"button\" onclick=\"popView('".$rs->fields["manu_mtra_stock_detail_seqno"]."');\" class=\"btn btn-sm btn-default\">수정</button>";
    else
        $btn = "";
...
$param["dvs"] = "COUNT";
$count_rs = $dao->selectMtraStockMngDetailList($conn, $param);
