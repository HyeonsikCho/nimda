...
class EngineDAO extends CommonDAO {
     ...
     * @param $conn  = 디비 커넥션
     * @param $param = 파라메터
     * $param['date'] = ci.issue_date or ci.use_date
     *
     * @return
     */
    function selectCpStats($conn, $param) {
        ...
        if ($param['date'] === "ci.use_date") {
            $query .= "\n    ,sum(use_price) use_price";
        }

        $query .= "\n   FROM  cp c";     
        $query .= "\n        ,cp_issue ci";
        $query .= "\n  WHERE c.cp_seqno = ci.cp_seqno ";          

        //테스트
        /*
        $query .= "\n    AND DATE_FORMAT(". $param['date'] .", '%Y-%c-%d %H:%i:%s') ";
        $query .= " >= DATE_FORMAT(sysdate(), '%Y-%c-%d 00:00:00')";
        $query .= "\n    AND DATE_FORMAT(". $param['date'] .", '%Y-%c-%d %H:%i:%s') ";
        $query .= " <= DATE_FORMAT(sysdate(), '%Y-%c-%d 23:59:59')";
        */
        
        //리얼
        $query .= "\n    AND DATE_FORMAT(". $param['date'] .", '%Y-%c-%d %H:%i:%s')";
        $query .= " >= DATE_FORMAT(DATE_SUB(sysdate(), interval 1 day), '%Y-%c-%d 00:00:00')";
        $query .= "\n    AND DATE_FORMAT(". $param['date'] .", '%Y-%c-%d %H:%i:%s')";
        $query .= " <= DATE_FORMAT(DATE_SUB(sysdate(), interval 1 day), '%Y-%c-%d 23:59:59')";
        if ($param['date'] === "ci.use_date") {
            $query .= "\n    AND ci.use_yn ='Y' ";
            $query .= "\n GROUP BY ci.cp_seqno ";
        }
     ...
     * @return
     */
    function countCpStats($conn, $param) {
    
        if (!$conn) {
            echo "master connection failed\n";
            return false;
        }
        $query .= "\n SELECT count(cp_seqno) as cnt";
        $query .= "\n   FROM cp_issue";
        $query .= "\n  WHERE cp_seqno = ". $param["cp_seqno"];
    ...
    /**
     * @brief 쿠폰 사용여부를 N 으로 변경
     * 소멸기간이 지남에 따라 사용여부 N으로 변경
     * @detail 
     *
     * @param $conn  = 디비 커넥션
     *
    ...
    function updateCpUseYN($conn) {
        ...
        $query  = "\n UPDATE cp";
        $query .= "\n    SET use_yn = 'N'";
        $query .= "\n  WHERE DATE_FORMAT(DATE_SUB(sysdate(), interval 2 hour), '%Y-%c-%d %H:00:00') > DATE_FORMAT(cp_extinct_date, '%Y-%c-%d 00:00:00')";
        
        return $conn->Execute($query);
    }

    /**
     * @brief 월 쿠폰 통계 데이터 값 구하기
     * 일_쿠폰_사용_통계 테이블의 데이터를 합계
     *
     * @param $conn  = 디비 커넥션
     * $param["cp_seqno"] = "cp_seqno"
     * $param["sum"] = "issue_count or use_count or use_price"
     * $param["year"] = "year"
     * $param["mon"] = "mon"
     *
     * @return
     */
    function selectSumCpUse($conn, $param) {
        if (!$this->connectionCheck($conn)) return false;

        $except_arr = array("sum" => true);

        $param = $this->parameterArrayEscape($conn, $param, $except_arr);

        $query  = "\n SELECT sum(%s) as sum";
        $query .= "\n   FROM day_cp_use_stats";
        $query .= "\n  WHERE year = %s";
        $query .= "\n    AND mon = %s";
        $query .= "\n    AND cp_seqno = %s";
        
        $query  = sprintf($query, $param["sum"]
                                , $param["year"]
                                , $param["mon"]
                                , $param["cp_seqno"]);
    ...
    /**
     * @brief 주문_공통 테이블에서 한달동안의 회원이 결제한 총금액 및 회원번호 조회
     *
     * @param $conn  = 디비 커넥션
     *
     * @return 
