 ...
 class CLS_Order {
	 ...
	 ***********************************************************************************/
	 ...
	 function getOrderInfoDataValue($conn, $OrderNum) {
		...
		$this->sql .= "\n,od.receipt_finish_date, od.tomson_yn, od.stan_name ";
		$this->sql .= "\n  FROM "._TBL_ORDER." AS oc, "._TBL_ORDER_DETAIL." AS od, "._TBL_ORDER_DETAIL_COUNT_FILE." AS odcf ";
		$this->sql .= "\n WHERE oc.order_common_seqno = od.order_common_seqno AND od.order_detail_seqno = odcf.order_detail_seqno ";
		...
		if ($rs && !$rs->EOF) {
			...
			$nRes['oc.dlvr_produce_dvs'] = $rs->fields['dlvr_produce_dvs'];                                  // 배송판구분 *
	 ...
	 ***********************************************************************************/
	 ...
	 function getLocalBasicAfterDataValue($conn, $OrderNum) {
		 ...
		 $this->sql = "SELECT after_name FROM "._TBL_ORDER_AFTER_HISTORY." ";
  ...
  ***********************************************************************************/
	 ...
	 ***********************************************************************************/
	 ...
	 function getLocalMemberDataValue($conn, $mb_idx) {
		...
		$this->sql = "SELECT member_dvs, member_id, member_name, office_nick, tel_num, cell_num, addr ";
		...
		$rs = $conn->Execute($this->sql);
		 ...
		 if ($rs && !$rs->EOF) {
			 ...
			 $nRes['mb_phone'] = $rs->fields['tel_num'];
			 $nRes['mb_mobile'] = $rs->fields['cell_num'];
			 $nRes['mb_addr'] = $rs->fields['addr'];
		 } else if ($rs->EOF) {
			 $nRes = "FAILED";
		 } else {
			 $nRes = "ERROR";
	 ...
	 ***********************************************************************************/
	 ...
	 function getLocalPaperNDCBDataValue($conn, $OrderNum) {
		 $this->Init();
		 $OrderNum = substr($OrderNum, 0, strlen($OrderNum) - 2);
	 ...
	 ***********************************************************************************/
	 ...
	 function getLocalDivDataValue($conn, $OrderNum) {
		 ...
		 $this->sql = "SELECT oi.color_name, oi.color_code, oi.nick ";
