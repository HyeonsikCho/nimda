...
// Settings
    ...
    $upload_path = SITE_DEFAULT_AFTER_OP_WORK_FILE;
...
//$file_name = generateFilename($_FILES[$upload_name]['name']);
    ...
    $file_name = "after_" . $fb->form("order_detail_dvs_num") . '.' . $ext;
...
// Process the file
	...
	*/
    ...
    $param["col"]["file_path"] = $save_path;
    $param["col"]["size"] = $file_size;

    if ($sel_rs->EOF == 1) {
        $rs = $dao->insertData($conn, $param);

        $after_op_work_file_seqno = $conn->Insert_ID();
    } else {
        ...
        $after_op_work_file_seqno = $sel_rs->fields["after_op_work_file_seqno"];   
    }
    ...
    showResult(true, $after_op_work_file_seqno, "upload completed");
