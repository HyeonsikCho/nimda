...
// Process the file
	...
	*/
    ...
    if ($fb->form("depth1")) {
        $after_name .= "-" . $fb->form("depth1");
    }

    if ($fb->form("depth2")) {
        $after_name .= "-" . $fb->form("depth2");
    }
    
    if ($fb->form("depth3")) {
        $after_name .= "-" . $fb->form("depth3");
    }
    ...
    $file_seqno .= $conn->Insert_ID("after_op_work_file") . '|';
